...
The `config.bin` API has been removed. Custom commands are now registered through `config.cli.commands` and use the same schema-backed Commander API as Payload's built-in commands.
...
export const seedCommand = defineCLICommand({
  ...
  input: z.strictObject({
    clear: z
      .boolean()
      .default(false)
      .describe('Delete existing documents first.'),
  }),
...
Payload adds built-in command import references to `cli.commands` while sanitizing the config, then applies project entries over those defaults. All built-ins are named exports from `payload/cli/builtin`, and Payload imports a referenced module only once even when it provides several commands. The sanitized map therefore contains the complete local CLI, and built-in and project commands use the same loader. Custom commands appear in `payload --help` and receive Standard Schema validation and Commander help generation. A command with the same map key as a built-in command replaces it, while `false` disables that command. Set `cli: false` to disable the entire Payload CLI. Commands also appear with a generated JSON Schema in `payload help --json`, and every command accepts the schema object directly through `--input <json|@file|->`. Command names are case-sensitive. Because custom commands must be registered before Commander parses input, the Payload config is now loaded before displaying root CLI help.

See the [Payload CLI documentation](../configuration/cli) for the complete API.
...
### Lexical: `EXPERIMENTAL_TableFeature` renamed to `TableFeature`

The table feature is no longer experimental, so it drops its prefix.
...
### Jobs: custom database adapters must implement `updateJobs`

Payload now uses a unique `processingToken` when claiming jobs so multiple workers cannot run the same job. This adds an internal, nullable `processingToken` field to the `payload-jobs` collection. Existing SQL databases need a migration that adds this column. MongoDB does not require a migration.
...
### `@payloadcms/storage-uploadthing` has been removed
...
| Allowed methods | `GET,PUT,OPTIONS` (add `HEAD` if reading blobs in-browser) |
...
### `@payloadcms/ui/scss` export removed and legacy `--theme-*` tokens retired
...
The `--theme-elevation-*` scale was removed. Use the semantic `--color-*` tokens (e.g. `--color-bg`, `--color-text`, `--color-border`, `--color-icon`) or the raw `--ramp-*` palette instead. See [Customizing CSS](../admin/customizing-css) for the full list of available tokens.
